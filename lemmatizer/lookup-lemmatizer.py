...
for line in train_data:
    ...
    if re.search ('\t' , line):
        ...
        elif lemma not in lemma_count[form]:
            lemma_count[form][lemma] = 1
        else:
            lemma_count[form][lemma] += 1
...
for form in lemma_count.keys():
        ...
        if(len(lemma_count[form]) > 1):
            ...
            max_value = list(lemma_count[form].values())[0]
            commonLemma = list(lemma_count[form].keys())[0]
            for key in lemma_count[form]:
                if(lemma_count[form][key] > max_value):
                    max_value = lemma_count[form][key]
                    commonLemma = key
                if(form == key):
                    training_counts['Identity tokens'] += lemma_count[form][key]
                training_counts['Wordform tokens'] += lemma_count[form][key]
                training_counts['Ambiguous tokens'] += lemma_count[form][key]
            training_counts['Ambiguous most common tokens'] += max_value
            lemma_max[form] = commonLemma
        else:
            lemma_max[form] = list(lemma_count[form].keys())[0]
            training_counts['Wordform tokens'] += list(lemma_count[form].values())[0]
            ...
            if(form == lemma_max[form]):
                training_counts['Identity tokens'] += list(lemma_count[form].values())[0]
